...
class StreamClient {
  ...
  Future<StreamManifest> getManifest(dynamic videoId,
    ...
    // Use tv first (no signature decoding needed, works on restricted videos),
    // then ios (reliable audio-only), then androidVr (high-quality fallback).
    ...
    for (final client in clients) {
      ...
      try {
        ...
        // Stop trying more clients once we have at least one stream
        if (uniqueStreams.isNotEmpty) break;
      } catch (e, s) {
        _logger.severe(
